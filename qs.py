...
class Application(tk.Frame):
	...
	# radio button for SMP or MPP
        ...
        # listbox for showning the pushed jobs
        #self.scrollbar = Scrollbar(frame6)
        #self.scrollbar.pack(side = RIGHT, fill = Y)
        self.listbox2 = Listbox(frame6, height = 5, width = 20)
        self.listbox2.grid(row = 1, column = 1)
        self.listbox3 = Listbox(frame6, height = 5, width = 100) #, xscrollcommand = self.scrollbar.set)
        self.listbox3.grid(row = 1, column = 2)
        #self.listbox3.config(xscrollcommand.scrollbar.set)
        #scrollbar.config(commmand = listbox.xvuew)

        #self.scrollbar.config(command = self.listbox3.xview)

        # delete job button
        self.deletejob =  tk.Button(frame6, text="Delete job", command = self.deletejob)
        self.deletejob.grid(row = 1, column = 3)
...
#==========================================================================================
    def deletejob(self):
        ...
        # show IndexError
        try:
            self.index = int(self.listbox3.curselection()[0])
        except IndexError:
            messagebox.showinfo("","Please speficy the job to delete")
        else:
            self.selection = self.listbox3.curselection()
            self.listbox2.delete(self.selection, self.selection)
            self.listbox3.delete(self.selection, self.selection)
#==========================================================================================
    def pushjob(self):
